...
void set_max_type(const char* name, DataTypes_t t) {
    ...
    syserr("set_max_type: symbol not found");
}
...
bool is_maxed(const char* name, DataTypes_t t) {
    for (Scope_t *it = scope_top(); it; it = it->parent) {
        ...
        HASH_FIND_STR(it->symbols, name, v);
        if (v) {
            return v->max_type != UNKNOWN && t > v->max_type;
        }
    }
    syserr("is_maxed: symbol not found");
    return false;
}
...
void update_max_type(const char* name, DataTypes_t t) {
    for (Scope_t *it = scope_top(); it; it = it->parent) {
        ...
        HASH_FIND_STR(it->symbols, name, v);
        if (v) {
            if (v->max_type == UNKNOWN || t > v->max_type) {
                v->max_type = t;
                printf("updated: %s", data_type_to_str(t));
            }
            return;
        }
    }
    syserr("update_max_type: symbol not found");
}
