...
Value default_step(DataTypes_t type) {
    ...
    switch (type) {
        ...
        case UF32: step.fnum = 1.0f; break;
        case UF64: step.lfnum = 1.0; break;
...
bool step_is_positive(DataTypes_t type, Value step) {
    switch (type) {
        ...
        case UF32: return step.fnum > 0;
        case UF64: return step.lfnum > 0;
...
bool step_is_zero(DataTypes_t type, Value step) {
    switch (type) {
        ...
        case UF32: return fabsf(step.fnum) < 1e-12f;
        case UF64: return fabs(step.lfnum) < 1e-12;
...
bool should_continue_for(DataTypes_t type, Value cur, Value end, Value step) {
    if (step_is_positive(type, step)) {
        switch (type) {
            ...
            case UF32: return cur.fnum < end.fnum;
            case UF64: return cur.lfnum < end.lfnum;
...
Value add_step_for(DataTypes_t type, Value cur, Value step) {
    ...
    switch (type) {
        ...
        case UF32: next.fnum += step.fnum; break;
        case UF64: next.lfnum += step.lfnum; break;
