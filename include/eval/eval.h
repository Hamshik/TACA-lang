...
#define FP_CASES(field, a, b, POWF, MODF)                                      \
  ...
  case OP_DIV:                                                                 \
    return (TQValue){.field = (a) / (b)};                                      \
  case OP_POW:                                                                 \
    return (TQValue){.field = POWF((a), (b))};                                 \
  case OP_MOD:                                                                 \
    return (TQValue){.field = MODF((a), (b))};                                 \
  case OP_EQ:                                                                  \
    return (TQValue){.bval = (a) == (b)};                                      \
  case OP_NEQ:                                                                 \
    return (TQValue){.bval = (a) != (b)};                                      \
...
#define UNOP_CASES(field, operand)                                             \
  ...
  case OP_POS:                                                                 \
    ...
    break;                                                                     \
  case OP_INC:                                                                 \
    result->field = ((int)operand->field) + 1;                                 \
    break;                                                                     \
  case OP_DEC:                                                                 \
    result->field = ((int)operand->field) - 1;                                 \
    break;                                                                     \
  case OP_BITNOT:                                                              \
    result->i128 = ~operand->i128;                                             \
    break
...
TypedValue eval_call(ASTNode_t *node, bool g_returning,
                     TypedValue g_return_value);

TypedValue eval_for(ASTNode_t *node, bool g_returning,
                    TypedValue g_return_value);
...
void set_var_at(int frame_id, const char *name, TQValue *val,
