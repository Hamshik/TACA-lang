...
extern "C" int codegen(ASTNode_t* root, const char *ll_path, char **ir_out) {
    ...
    walk = [&](ASTNode_t *n){
        if (!n) return;
        if (n->kind == AST_FN) { emit_function(n, mod, ctx); return; }
        if (n->kind == AST_SEQ) { walk(n->seq.a); walk(n->seq.b); return; }
    ...
    Function *initFn = Function::Create(ftInit, Function::InternalLinkage, "init", mod);
    {
        ...
        std::function<void(ASTNode_t*)> emit_nonfn = [&](ASTNode_t *n){
            if (!n) return;
            if (n->kind == AST_FN) return;
            if (n->kind == AST_SEQ) { emit_nonfn(n->seq.a); emit_nonfn(n->seq.b); return; }
            emit_expr(n, ctx, ib, entryB, locals);
        };
        emit_nonfn(root);
        if (!entry->getTerminator()) ib.CreateRetVoid();
    }
    ...
    FunctionType *ftStub = FunctionType::get(b.getInt32Ty(), false);
    Function *stub = Function::Create(ftStub, Function::ExternalLinkage, "entrypoint", mod);
    ...
    if (userMain->getReturnType()->isVoidTy())
        ...
        b.CreateRet(b.CreateIntCast(retv, b.getInt32Ty(), true));
    ...
    if (ll_path) {
        ...
        if (ec) {
            std::cerr << "Failed to open " << ll_path << ": " << ec.message() << std::endl;
            return 1;
        }
        out << ir;
        out.flush();
