...
bool step_is_zero(DataTypes_t type, TQValue step) {
  ...
  case U128:
    return step.u128 == 0;
  case F32:
    return fabsf(step.f32) < 1e-12f;
  case F64:
    return fabs(step.f64) < 1e-12;
  case F128:
    return fabsl(step.f128) < 1e-18L;
  case UF32:
    return fabsf(step.f32) < 1e-12f;
  case UF64:
    return fabs(step.f64) < 1e-12;
...
bool should_continue_for(DataTypes_t type, TQValue cur, TQValue end,
                         TQValue step) {
  if (step_is_positive(type, step)) {
    switch (type) {
    case I8:
      return cur.i8 < end.i8;
    case I16:
      return cur.i16 < end.i16;
    case I32:
      return cur.i32 < end.i32;
    case I128:
      return cur.i128 < end.i128;
    case U8:
      return cur.u8 < end.u8;
    case U16:
      return cur.u16 < end.u16;
    case U32:
      return cur.u32 < end.u32;
    case U64:
      return cur.u64 < end.u64;
    case U128:
      return cur.u128 < end.u128;
    case F32:
      return cur.f32 < end.f32;
    case F64:
      return cur.f64 < end.f64;
    case F128:
      return cur.f128 < end.f128;
    case UF32:
      return cur.f32 < end.f32;
    case UF64:
      return cur.f64 < end.f64;
...
TQValue add_step_for(DataTypes_t type, TQValue cur, TQValue step) {
  ...
  case I128:
    ...
    break;
  case U8:
    next.u8 = (uint8_t)(next.u8 + step.u8);
    break;
  case U16:
    next.u16 = (uint16_t)(next.u16 + step.u16);
    break;
  case U32:
    next.u32 += step.u32;
    break;
...
TypedValue eval_for(ASTNode_t *node, bool g_returning, TypedValue g_return_value) {
  ...
  TypedValue stept =
      node->fornode.step
          ? TQcast_typed(ast_eval(node->fornode.step), loop_type, node->line,
                         node->col, node->pos)
  ...
  while (should_continue_for(
      loop_type,
      TQruntime_env_get(loop_name, loop_type, node->line, node->col, node->pos),
      endv_cast, stepv)) {
    last = ast_eval(node->fornode.body);
    if (g_returning)
      return g_return_value;
    TQValue cur = TQruntime_env_get(loop_name, loop_type, node->line, node->col,
                                    node->pos);
